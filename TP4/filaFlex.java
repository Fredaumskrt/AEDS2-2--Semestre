...
class celula {
    ...
    public celula(classGames elemento) {
...
class Fila {
    ...
    public Fila(int tamanho) {
        this.array = new classGames[tamanho + 1];
        this.tamanho = this.primeiro = this.ultimo = 0;
    }

    
    public void inserir(classGames game) {
    ...
    public classGames remover() throws Exception {
        ...
        classGames game = this.array[this.primeiro];
    ...
    public int media() {
        ...
        int soma = 0;
        while (i != this.ultimo) {
            soma += this.get(i).getavg_pt();
            i = (i + 1) % this.array.length;
        }
        return Math.round(soma * 1.0f / this.tamanho);
    }
    ...
    public classGames get(int posicao) {
        return this.array[(posicao) % this.array.length];
    }
}
...
class filaFlex {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < nextEntry; i++) {
            ...
            begin[count] = MyIO.readLine();
             String action = begin[count].charAt(0) + "";

            if (action.compareTo("I") == 0) {
                receiv = begin[count].substring(2);
                String gamesContent = findGameByID(classGames.pastaClasseGames, receiv);
                gamesT.readapp_id(gamesContent);
                test.inserir(gamesT);
                // System.out.println(test.media());java 
