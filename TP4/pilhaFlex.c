...
void subcommand(char s[], char sub[], int p, int l) {
   int c = 0;
...
int main()
{
    ...
    for (int i = 0; i < n_ops; i++)
    {
        ...
        {
            char num_game[15];
            subcommand(line_in, num_game, 2, strlen(line_in));
            int app_id = atoi(num_game);
            Game found = game_search(app_id);
            inserir(found);
        }
