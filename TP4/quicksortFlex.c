...
void quicksortRec(int esq, int dir) {
    ...
    Game pivo = caminhaPosic((dir+esq)/2).elemento;
    while (i <= j) {
        comparacoes++;
        Game filmeI=celulaI.elemento;
        Game filmeJ=celulaJ.elemento;
        while (strcmp(filmeI.release_date, pivo.release_date)<0){
            i++;
            comparacoes++;
            //celulaI=*celulaI.prox;
        } 
        while (strcmp(filmeJ.release_date, pivo.release_date)>0){
            j--;
            comparacoes++;
            //celulaJ=*celulaJ.ant;
        } 
...
int main() {
    ...
    // Execute operations
    int n_ops; // inteiro 
    scanf("%i", &n_ops); // sei la que disgraca eh essa
